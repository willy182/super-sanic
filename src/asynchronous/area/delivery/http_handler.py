...
from src.asynchronous.area.usecase.usecase_area import ListAllAreaUsecase, ListAllOnlyAreaUsecase
...
@bp_area_async.route('/only-database', methods=['GET'])
async def index(request):
    request_dict = RequestSanicDict(request)
    validator = JSONSchemaValidator()
    ...
    repo_init = AreaRepositoryPSQL(db=request.app.db, tracer=request.app.tracer)
